...
alias gglg-sig="git log --show-signature --graph --abbrev-commit --decorate --format=format:'%C(bold blue)%h%C(reset) - %C(bold green)(%ar)%C(reset) %C(white)%s%C(reset) %C(dim white)- %an%C(reset)%C(auto)%d%C(reset)' --all" 
...
alias ggi="git init"
alias ggd="git diff --color --word-diff=color --unified=5 --patience --stat"
